...
Username: root
Password: root
צריך לכניס את שניים התחלה
# file_contactbook = "contactbook.db"
"""
file_contactbook = path+r"\contactbook.db"  # database קובץ
#'r עובד בשל עושה "\c"'
print(file_contactbook)
...
class ContactManager:
    def __init__(self, root):
        ...
        Button(btnFrame, text='Add record', font='arial 12 bold',
               bg='black', fg='white', width=9, command=self.addrecord).grid(row=0, column=0, padx=10, pady=10)
        # Create a button to update the Details
        Button(btnFrame, text='Update', font='arial 12 bold',
               bg='black', fg='white', width=9, command=self.update).grid(row=0, column=1, padx=8, pady=10)
        # Create a button to delete the entry
        Button(btnFrame, text='Delete', font='arial 12 bold',
               bg='black', fg='white', width=9, command=self.delete).grid(row=0, column=2, padx=8, pady=10)
        # Create a button to reset the fields

        Button(btnFrame, text='Reset', font='arial 12 bold',
               bg='black', fg='white', width=9, command=self.reset).grid(row=0, column=3, padx=8, pady=10)
        # Create a button to exit the clos

        Button(btnFrame, text='exit', font='arial 12 bold',
               bg='black', fg='white', width=9, command=self.reset.destroy).grid(row=0, column=4, padx=8, pady=10)
    ...
    def addrecord(self):
        if self.firstname.get() == '' or self.lastname.get() == '' or self.mobile.get() == '' or self.addr.get() == '' or self.pin.get() == '':
            ...
            con = sqlite3.connect(file_contactbook)
            cur = con.cursor()
...
class Login():
    def __init__(self, root):
        ...
        Button(self.root, text="Login", fg='dark blue', command=self.login).grid(
            columnspan=4, row=2, column=0, padx=0, pady=0)
        Button(self.root, text="exit", fg='dark blue', command=root.destroy).grid(
