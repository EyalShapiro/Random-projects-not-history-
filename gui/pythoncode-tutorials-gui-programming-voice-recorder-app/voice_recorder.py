...
import os
# https://www.thepythoncode.com/article/make-a-gui-voice-recorder-python
####################
...
def record_voice():
    ...
    try:
        ...
        counter = 0
        # קובץ שמע
        file_wav = "C:\Eyal\Codes\py\gui\\tk\pythoncode-tutorials-gui-programming-voice-recorder-app\\recording.wav"
        ...
        for file in os.listdir():
            ...
            if file == file_wav:
                ...
                new_name = 'recording-' + str(current_time.hour) + '.' + str(
                    current_time.minute) + '.' + str(current_time.second) + ext
                # renaming the file
...
window.iconbitmap(window, file_ico)
...
button_style.configure('TButton', foreground='red', font='DotumChe')
...
# loading the logo
logo = PhotoImage(file=file_recorder)
...
duration_label = ttk.Label(
    window, text='Enter Recording Duration in Sec(int):', style='TLabel')
# creating a ttk entry
duration_entry = ttk.Entry(window, width=76, style='TEntry')

# adding the label to the canvas
canvas.create_window(240, 300, window=duration_label)
# adding the entry to the canvas
canvas.create_window(250, 325, window=duration_entry)


# creating the empty label for displaying download progress
progress_label = ttk.Label(window, text='')
# creating the button
record_button = ttk.Button(window, text='Record',
                           style='TButton', command=recording_thread)

# adding the label to the canvas
canvas.create_window(242, 365, window=progress_label)
