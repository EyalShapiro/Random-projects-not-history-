...
import shutil
# import the mesassage box for displaying the messages
...
def showFilePath():
    files = fd.askopenfilename(title="Select the file to rename", filetypes=[
                               ("All files", "*.*")])
    return files
...
def NameSubmit():
    ...
    newFileName = os.path.join(os.path.dirname(
        fileName), renameName + os.path.splitext(fileName)[1])
    os.rename(fileName, newFileName)
    mb.showinfo(title="File Renamed!",
                message="The selected file has been renamed.")
...
def moveAFolder():
    ...
    try:
        ...
        mb.showinfo(
            "Folder moved!", 'The selected folder has been moved to the desired Location')
    except:
        mb.showerror(
            'Error!', 'The Folder cannot be moved. Make sure that the destination exists')
